...
const STATUS_COLORS: Record<string, { bg: string; color: string; border: string }> = {
  ...
  assigned:    { bg: "rgba(167,139,250,0.12)", color: "#a78bfa",  border: "rgba(167,139,250,0.3)" },
...
export default function BatterySwapAdminPage() {
  ...
  const StatCard = ({ label, value, color }: { label: string; value: number; color: string }) => (
    <div style={{ background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 12, padding: "16px 20px", flex: "1 1 120px" }}>
      <p style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 6px" }}>{label}</p>
  ...
  return (
    <div style={{ padding: "24px 28px", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", margin: 0 }}>🔋 Battery Swap Requests</h1>
          <p style={{ color: "#64748b", fontSize: 13, margin: "4px 0 0" }}>Manage and process all battery swap service requests</p>
        </div>
        <button onClick={loadData} style={{
          padding: "8px 16px", background: "transparent", border: "1px solid #1e2d40",
      ...
      {stats && (
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          <StatCard label="Pending" value={stats.pending} color="#f97316" />
          <StatCard label="Confirmed" value={stats.confirmed} color="#00d4ff" />
          <StatCard label="Assigned" value={stats.assigned} color="#a78bfa" />
          ...
          <StatCard label="Done Today" value={stats.completed_today} color="#10b981" />
          {stats.unread > 0 && <StatCard label="Unread" value={stats.unread} color="#ef4444" />}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <input
          ...
          style={{
            flex: "1 1 200px", background: "#0f172a", border: "1px solid #1e2d40",
            borderRadius: 8, color: "#f1f5f9", fontSize: 13, padding: "8px 12px", outline: "none",
          }}
        />
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
          style={{ background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 8, color: "#f1f5f9", fontSize: 13, padding: "8px 12px", cursor: "pointer" }}
        >
        ...
        <button onClick={loadData} style={{
          padding: "8px 16px", background: "#00d4ff", border: "none", borderRadius: 8,
          color: "#0a0f1e", fontSize: 13, fontWeight: 700, cursor: "pointer",
        }}>
          Apply
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: selectedId ? "0 0 55%" : "1", overflow: "auto" }}>
          <div style={{ background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 12, overflow: "hidden" }}>
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "#64748b" }}>Loading...</div>
            ) : swaps.length === 0 ? (
              <div style={{ padding: 40, textAlign: "center", color: "#64748b" }}>No battery swap requests found.</div>
            ) : (
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid #1e2d40" }}>
                    {["Token", "Customer", "Battery", "Date", "Slot", "Location", "Status", ""].map((h) => (
                      <th key={h} style={{ padding: "10px 12px", fontSize: 11, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", textAlign: "left", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {swaps.map((sw) => (
                    <tr
                      ...
                      style={{
                        ...
                        background: selectedId === sw.id ? "rgba(0,212,255,0.05)" : "transparent",
                        transition: "background 0.15s",
                      }}
                      onMouseEnter={(e) => { if (selectedId !== sw.id) (e.currentTarget as HTMLTableRowElement).style.background = "rgba(255,255,255,0.02)"; }}
                      onMouseLeave={(e) => { if (selectedId !== sw.id) (e.currentTarget as HTMLTableRowElement).style.background = "transparent"; }}
                    >
                      <td style={{ padding: "10px 12px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          {!sw.is_read && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00d4ff", flexShrink: 0 }} />}
                          <span style={{ fontFamily: "monospace", color: "#00d4ff", fontSize: 12, fontWeight: 700 }}>{sw.token_number}</span>
                        </div>
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <p style={{ color: "#f1f5f9", fontSize: 13, fontWeight: 600, margin: 0 }}>{sw.name}</p>
                        <p style={{ color: "#64748b", fontSize: 11, margin: 0 }}>{sw.phone}</p>
                      </td>
                      <td style={{ padding: "10px 12px", color: "#94a3b8", fontSize: 12 }}>
                        <p style={{ margin: 0 }}>{sw.battery_capacity}</p>
                        <p style={{ margin: 0, color: "#64748b", fontSize: 11 }}>{sw.battery_type}</p>
                      </td>
                      <td style={{ padding: "10px 12px", color: "#94a3b8", fontSize: 12, whiteSpace: "nowrap" }}>{formatDate(sw.preferred_date)}</td>
                      <td style={{ padding: "10px 12px", color: "#64748b", fontSize: 11 }}>{sw.preferred_time_slot.replace(" ", "\n")}</td>
                      <td style={{ padding: "10px 12px", color: "#64748b", fontSize: 11 }}>{sw.swap_location}</td>
                      <td style={{ padding: "10px 12px" }}><StatusBadge status={sw.status} /></td>
                      <td style={{ padding: "10px 12px" }}>
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(sw.id); }} style={{
                          background: "transparent", border: "none", color: "#64748b", cursor: "pointer", fontSize: 14, padding: 4,
                        }} title="Delete">🗑</button>
                      </td>
                    </tr>
                  ))}
        ...
        {/* Detail panel */}
        {selectedId && (
          <div style={{ flex: "0 0 44%", background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 16, overflow: "hidden" }}>
            {detailLoading ? (
              <div style={{ padding: 40, textAlign: "center", color: "#64748b" }}>Loading details...</div>
            ) : detail ? (
              <div>
                {/* Panel header */}
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #1e2d40", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <p style={{ color: "#64748b", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>Swap Token</p>
                    <p style={{ color: "#00d4ff", fontFamily: "monospace", fontSize: 18, fontWeight: 900, margin: 0 }}>{detail.token_number}</p>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <StatusBadge status={detail.status} />
                    <button onClick={() => { setSelectedId(null); setDetail(null); }}
                      style={{ background: "transparent", border: "none", color: "#64748b", cursor: "pointer", fontSize: 18, padding: 4 }}>×</button>
                  </div>
                </div>
                ...
                <div style={{ padding: "16px 20px", maxHeight: "80vh", overflowY: "auto" }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <p style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" }}>Customer</p>
                    <div style={{ background: "#0a0f1e", borderRadius: 10, padding: 14 }}>
                      <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(0,212,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 800, color: "#00d4ff", marginBottom: 10 }}>
                      ...
                      {[["Name", detail.name], ["Phone", detail.phone], ["Email", detail.email || "—"], ["City", detail.city], ["Address", detail.address]].map(([k, v]) => (
                        <div key={k as string} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid rgba(30,45,64,0.5)" }}>
                          ...
                          <span style={{ color: "#f1f5f9", fontSize: 12, fontWeight: 500, textAlign: "right", maxWidth: "60%" }}>{v}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Battery info */}
                  <div style={{ marginBottom: 16 }}>
                    <p style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" }}>Battery Details</p>
                    <div style={{ background: "#0a0f1e", borderRadius: 10, padding: 14 }}>
                      ...
                      {detail.any_physical_damage && (
                        <div style={{ marginTop: 8, padding: "8px 10px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 6 }}>
                          <p style={{ color: "#ef4444", fontSize: 12, fontWeight: 700, margin: "0 0 4px" }}>⚠️ Physical Damage Reported</p>
                          {detail.damage_description && <p style={{ color: "#94a3b8", fontSize: 12, margin: 0 }}>{detail.damage_description}</p>}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Schedule */}
                  <div style={{ marginBottom: 16 }}>
                    <p style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" }}>Schedule Requested</p>
                    <div style={{ background: "#0a0f1e", borderRadius: 10, padding: 14 }}>
                      {[
                  ...
                  {/* Admin update form */}
                  <div style={{ background: "#0a0f1e", border: "1px solid rgba(0,212,255,0.15)", borderRadius: 12, padding: 16 }}>
                    <p style={{ color: "#00d4ff", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 14px" }}>Admin Actions</p>
                    ...
                    {[
                      ...
                      { label: "Confirmed Date", content: (
                        <input type="date" value={updateDate} onChange={(e) => setUpdateDate(e.target.value)}
                          style={{ width: "100%", background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 6, color: "#f1f5f9", fontSize: 13, padding: "8px 10px", boxSizing: "border-box" as const }} />
                      )},
                      ...
                      { label: "Admin Notes", content: (
                        <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={2}
                          ...
                          style={{ width: "100%", background: "#0f172a", border: "1px solid #1e2d40", borderRadius: 6, color: "#f1f5f9", fontSize: 13, padding: "8px 10px", boxSizing: "border-box" as const, resize: "vertical" }} />
                      )},
                      ...
                      <div key={label} style={{ marginBottom: 12 }}>
                        <label style={{ fontSize: 11, color: "#64748b", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</label>
                        {content}
                      </div>
                    ))}

                    {saveMsg && (
                      <p style={{ fontSize: 12, color: saveMsg.startsWith("✓") ? "#10b981" : "#ef4444", margin: "0 0 10px" }}>{saveMsg}</p>
                    )}

                    <button onClick={handleUpdate} disabled={saving} style={{
                      width: "100%", padding: "10px", background: saving ? "#1e2d40" : "#00d4ff",
                      border: "none", borderRadius: 8, color: saving ? "#64748b" : "#0a0f1e",
      ...
      {photoOpen && detail?.battery_photo_url && (
        ...
        >
          <img
            src={`${BASE_URL}${detail.battery_photo_url}`}
            alt="Battery full"
            style={{ maxWidth: "90vw", maxHeight: "85vh", borderRadius: 12, border: "2px solid #1e2d40" }}
