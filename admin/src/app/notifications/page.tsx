  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 900 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-heading)", margin: "0 0 4px" }}>Notifications</h1>
          <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>{unreadCount} unread notification{unreadCount !== 1 ? "s" : ""}</p>
        </div>
        <button
          onClick={handleMarkAllRead}
          disabled={unreadCount === 0}
          style={{ padding: "9px 18px", borderRadius: 8, border: "1px solid rgba(92,103,149,0.2)", background: unreadCount === 0 ? "transparent" : "var(--green)", color: unreadCount === 0 ? "var(--text-muted)" : "var(--text-heading)", fontSize: 12, fontWeight: 600, cursor: unreadCount === 0 ? "default" : "pointer" }}
        >
          Mark all read
        </button>
      </div>

      <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid rgba(92,103,149,0.2)", flexWrap: "wrap" }}>
        {FILTERS.map(({ key, label }) => (
          <button
            key={key}
            onClick={() => setFilter(key)}
            style={{ padding: "10px 16px", background: "transparent", border: "none", borderBottom: filter === key ? "2px solid var(--navy)" : "2px solid transparent", color: filter === key ? "var(--navy)" : "var(--text-muted)", fontSize: 13, fontWeight: filter === key ? 700 : 400, cursor: "pointer", marginBottom: -1 }}
          >
            {label}
          </button>
        ))}
      ...
      {loading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[...Array(4)].map((_, i) => (
            <div key={i} style={{ height: 64, background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 12, opacity: 1 - i * 0.15 }} />
          ))}
        </div>
      ) : error ? (
        <div style={{ background: "rgba(255,92,92,0.06)", border: "1px solid rgba(255,92,92,0.25)", borderRadius: 14, padding: 32, textAlign: "center" }}>
          ...
          <button onClick={fetchNotifications} style={{ padding: "9px 20px", borderRadius: 8, border: "none", background: "var(--navy)", color: "var(--text-heading)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
            Retry
          </button>
        </div>
      ) : filtered.length === 0 ? (
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: n.is_read ? "var(--bg-card)" : "rgba(13,81,140,0.04)",
                  border: `1px solid ${n.is_read ? "rgba(92,103,149,0.2)" : `${color}30`}`,
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "var(--text-heading)", margin: "0 0 4px" }}>{n.title}</p>
                  <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 6px", lineHeight: 1.5 }}>{n.message}</p>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>{timeAgo(n.created_at)}</p>
                </div>
                {!n.is_read && <span style={{ position: "absolute", top: 16, right: 16, width: 8, height: 8, borderRadius: "50%", background: "var(--navy)" }} />}
              </div>
            );
          })}
        </div>
