  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 1100 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", margin: "0 0 4px" }}>Testimonials</h1>
          <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>Manage homepage customer testimonials</p>
        </div>
      ...
      {showForm && (
        <div style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 14, padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
            ...
            <span style={{ fontSize: 13, color: "#94a3b8" }}>Active (visible on homepage)</span>
          </div>
      ...
      {loading ? (
        <div style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 14, padding: 24 }}>
          {[...Array(3)].map((_, i) => (
            <div key={i} style={{ height: 60, background: "#0f172a", borderRadius: 8, marginBottom: 10, opacity: 1 - i * 0.2 }} />
          ))}
        </div>
      ) : sortedItems.length === 0 ? (
        <div style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 14, padding: 48, textAlign: "center" }}>
          <p style={{ fontSize: 14, color: "#64748b" }}>No testimonials yet</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {sortedItems.map((t, i) => (
            <div key={t.id} style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 14, padding: 18, display: "flex", gap: 16, alignItems: "flex-start" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <button onClick={() => move(t, -1)} disabled={i === 0 || busyId === t.id} style={{ background: "transparent", border: "1px solid #1e2d40", color: "#94a3b8", borderRadius: 6, width: 24, height: 24, cursor: i === 0 ? "not-allowed" : "pointer", fontSize: 11 }}>↑</button>
                <button onClick={() => move(t, 1)} disabled={i === sortedItems.length - 1 || busyId === t.id} style={{ background: "transparent", border: "1px solid #1e2d40", color: "#94a3b8", borderRadius: 6, width: 24, height: 24, cursor: i === sortedItems.length - 1 ? "not-allowed" : "pointer", fontSize: 11 }}>↓</button>
              </div>
              <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(0,212,255,0.12)", border: "2px solid rgba(0,212,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#00d4ff", flexShrink: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 12, color: "#f97316", fontWeight: 700 }}>{t.rating} ★</span>
                  {t.product_used && (
                    <span style={{ fontSize: 11, color: "#00d4ff", background: "rgba(0,212,255,0.1)", padding: "2px 8px", borderRadius: 100, border: "1px solid rgba(0,212,255,0.3)" }}>
                      {t.product_used}
                    </span>
                  )}
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 100, background: t.is_active ? "rgba(16,185,129,0.1)" : "rgba(100,116,139,0.1)", color: t.is_active ? "#10b981" : "#64748b" }}>
                    {t.is_active ? "Active" : "Inactive"}
                  </span>
                </div>
                <p style={{ fontSize: 13, color: "#94a3b8", margin: 0, lineHeight: 1.6 }}>{t.comment}</p>
              </div>
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                <button onClick={() => openEdit(t)} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #1e2d40", background: "transparent", color: "#94a3b8", fontSize: 11, cursor: "pointer" }}>Edit</button>
                <button disabled={busyId === t.id} onClick={() => handleToggle(t.id)} style={{ padding: "6px 12px", borderRadius: 6, border: "none", background: t.is_active ? "rgba(249,115,22,0.1)" : "rgba(16,185,129,0.1)", color: t.is_active ? "#f97316" : "#10b981", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                  {t.is_active ? "Deactivate" : "Activate"}
                </button>
                <button disabled={busyId === t.id} onClick={() => handleDelete(t.id)} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #1e2d40", background: "transparent", color: "#ef4444", fontSize: 11, cursor: "pointer" }}>Delete</button>
              </div>
            </div>
          ))}
        </div>
