...
const blurBorder = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) =>
  (e.currentTarget.style.borderColor = "rgba(92,103,149,0.3)");
...
export default function SettingsPage() {
  ...
  const Toggle = ({ on, toggle }: { on: boolean; toggle: () => void }) => (
    <button onClick={toggle} type="button"
      style={{ width: 42, height: 22, borderRadius: 11, border: "none", cursor: "pointer", background: on ? "var(--navy)" : "rgba(92,103,149,0.2)", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
  ...
  const SavedBadge = ({ section }: { section: string }) => {
    if (saveError?.section === section) {
      return (
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "rgba(255,92,92,0.1)", border: "1px solid rgba(255,92,92,0.3)", borderRadius: 8, fontSize: 12, color: "var(--red)", fontWeight: 600 }}>
    ...
    if (saved === section) {
      return (
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "rgba(52,199,138,0.1)", border: "1px solid rgba(52,199,138,0.3)", borderRadius: 8, fontSize: 12, color: "var(--green)", fontWeight: 600 }}>
          <Check size={13} /> Saved!
        </div>
      );
    }
  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 900 }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-heading)", margin: "0 0 32px" }}>Settings</h1>

      {loadError && (
        <div style={{ padding: "10px 16px", background: "rgba(244,196,48,0.1)", border: "1px solid rgba(244,196,48,0.3)", borderRadius: 8, marginBottom: 20, fontSize: 12, color: "var(--gold)" }}>
          {loadError}
        </div>
      )}

      {/* COMPANY INFO */}
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Company Information</h2>
        ...
        <button onClick={saveCompany}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 24px", background: "var(--navy)", color: "var(--text-heading)", fontWeight: 700, fontSize: 13, borderRadius: 9, border: "none", cursor: "pointer", marginTop: 20 }}>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Business Hours</h2>
          <SavedBadge section="hours" />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {DAYS.map((day) => (
            <div key={day} style={{ display: "flex", alignItems: "center", gap: 16, padding: "10px 0", borderBottom: "1px solid rgba(92,103,149,0.2)" }}>
              <Toggle on={hours[day]?.open ?? false} toggle={() => setHour(day, { open: !hours[day]?.open })} />
              <span style={{ fontSize: 13, color: hours[day]?.open ? "var(--text-heading)" : "var(--text-muted)", width: 96, fontWeight: 500 }}>{day}</span>
              {hours[day]?.open ? (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input type="time" value={hours[day]?.from ?? "09:00"} onChange={(e) => setHour(day, { from: e.target.value })}
                    style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.3)", borderRadius: 6, padding: "6px 10px", color: "var(--text-heading)", fontSize: 12, outline: "none" }} />
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Service Areas</h2>
          <SavedBadge section="areas" />
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
          {areas.map((a) => (
            <div key={a} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", background: "rgba(13,81,140,0.08)", border: "1px solid rgba(13,81,140,0.2)", borderRadius: 100, fontSize: 13, color: "var(--navy)", fontWeight: 600 }}>
              {a}
              <button onClick={() => removeArea(a)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--text-muted)", display: "flex", padding: 0 }}><X size={12} /></button>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={addArea} style={{ padding: "10px 16px", borderRadius: 8, border: "none", background: "var(--navy)", color: "var(--text-heading)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            <Plus size={14} /> Add
          </button>
        </div>
        <button onClick={saveAreas}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 24px", background: "var(--navy)", color: "var(--text-heading)", fontWeight: 700, fontSize: 13, borderRadius: 9, border: "none", cursor: "pointer", marginTop: 16 }}>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Notification Settings</h2>
        ...
        {([
          ...
          <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid rgba(92,103,149,0.2)" }}>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Technicians</h2>
          <SavedBadge section="technicians" />
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
          {technicians.map((t) => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", background: "rgba(13,81,140,0.08)", border: "1px solid rgba(13,81,140,0.2)", borderRadius: 100, fontSize: 13, color: "var(--navy)", fontWeight: 600 }}>
              {t}
              <button onClick={() => removeTechnician(t)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--text-muted)", display: "flex", padding: 0 }}><X size={12} /></button>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={addTechnician} style={{ padding: "10px 16px", borderRadius: 8, border: "none", background: "var(--navy)", color: "var(--text-heading)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            <Plus size={14} /> Add
          </button>
        </div>
      ...
      {/* CHANGE PASSWORD */}
      <div style={{ background: "var(--bg-card)", border: "1px solid rgba(92,103,149,0.2)", borderRadius: 14, padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", margin: 0 }}>Change Admin Password</h2>
          <SavedBadge section="password" />
        </div>
        {pwError && (
          <div style={{ padding: "10px 14px", background: "rgba(255,92,92,0.08)", border: "1px solid rgba(255,92,92,0.25)", borderRadius: 8, marginBottom: 16, fontSize: 13, color: "var(--red)" }}>{pwError}</div>
        )}
        <form onSubmit={changePw} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <button type="submit" disabled={pwLoading}
            style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 24px", background: pwLoading ? "rgba(92,103,149,0.2)" : "var(--navy)", color: pwLoading ? "var(--text-muted)" : "var(--text-heading)", fontWeight: 700, fontSize: 13, borderRadius: 9, border: "none", cursor: pwLoading ? "not-allowed" : "pointer", alignSelf: "flex-start" }}>
