...
export function StatCardsSkeleton() {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 18, marginBottom: 32 }}>
      {[...Array(4)].map((_, i) => (
        <div key={i} style={{ background: "#ffffff", border: "1px solid #e8dfd0", borderRadius: 14, padding: "20px 22px" }}>
...
export function TableSkeleton({ rows = 6, cols = 5 }) {
  return (
    <div style={{ background: "#ffffff", border: "1px solid #e8dfd0", borderRadius: 14, overflow: "hidden" }}>
      <div style={{ background: "#f9f4ec", padding: "14px 16px", display: "flex", gap: 20 }}>
        {[...Array(cols)].map((_, i) => (
          <Box key={i} w={`${60 + i * 10}px`} h={12} />
        ))}
      </div>
      {[...Array(rows)].map((_, i) => (
        <div key={i} style={{ padding: "14px 16px", borderTop: "1px solid #e8dfd0", display: "flex", gap: 20, alignItems: "center" }}>
          {[...Array(cols)].map((_, j) => (
