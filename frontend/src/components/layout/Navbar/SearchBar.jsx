      ...
      {searchOpen ? (
        <div style={{ position: "relative" }}>
          ...
          {searchPreview.length > 0 && (
            <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, zIndex: 300, overflow: "hidden", boxShadow: "0 8px 32px rgba(26,15,0,0.18)" }}>
              {searchPreview.map((p) => (
                <Link
                  key={p.slug}
        ...
        <button
          ...
          style={{ padding: 8, color: "var(--text-muted)", background: "transparent", border: "none", cursor: "pointer", borderRadius: 8, transition: "color 0.2s", display: "flex", alignItems: "center" }}
