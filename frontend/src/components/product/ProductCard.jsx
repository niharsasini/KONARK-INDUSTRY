  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: `1px solid ${hovered ? "rgba(13,81,140,0.2)" : "rgba(13,81,140,0.07)"}`,
        borderRadius: 20,
        overflow: "hidden",
        display: "flex",
        ...
        boxShadow: hovered
          ? "0 8px 24px rgba(15,23,42,0.1), 0 0 0 1px rgba(13,81,140,0.08)"
    ...
    >
      ...
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.1)"; e.currentTarget.style.borderColor = "rgba(13,81,140,0.4)"; }}
        onMouseLeave={(e) => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.borderColor = "rgba(148,163,184,0.2)"; }}
        aria-label="Toggle wishlist"
      >
        <Heart size={14} color={inWishlist ? "#FF7043" : "#64748B"} fill={inWishlist ? "#FF7043" : "none"} />
      </button>

      {/* Top-left status badge — card stays fully opaque regardless of stock status */}
      {isUpcoming ? (
        <span style={{ position: "absolute", top: 12, left: 12, zIndex: 20, background: "rgba(148,163,184,0.8)", color: "#0F172A", fontSize: 10, fontWeight: 800, padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.8px" }}>
          Upcoming
        </span>
      ) : !inStock ? (
        <span style={{ position: "absolute", top: 12, left: 12, zIndex: 20, background: "#FF7043", color: "#0F172A", fontSize: 10, fontWeight: 800, padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.8px" }}>
          Out of Stock
        </span>
      ) : isNew ? (
        <span style={{ position: "absolute", top: 12, left: 12, zIndex: 20, background: "#34C78A", color: "#F5F7FF", fontSize: 10, fontWeight: 800, padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.8px" }}>
          New
        </span>
      ) : null}
            ...
            {images.map((_, i) => (
              <div
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: 3,
                  background: i === imgIndex ? "#0EA5E9" : "rgba(15,23,42,0.4)",
      ...
      <div className="product-card-body" style={{ padding: "16px 18px 18px", display: "flex", flexDirection: "column", flex: 1 }}>
        {product.category && (
          <p style={{ fontSize: 9, color: "#0EA5E9", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", margin: "0 0 4px" }}>
            {product.category}
          </p>
        )}

        <h3 className="product-card-name" style={{ fontSize: 16, fontWeight: 700, color: "#0F172A", margin: "0 0 6px", lineHeight: 1.3, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
          {product.name}
        </h3>

        {product.shortDescription && (
          <p style={{ fontSize: 12, color: "#94A3B8", margin: "0 0 12px", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
            {product.shortDescription}
          </p>
        )}
        ...
        <p className="product-card-price" style={{
          ...
          color: typeof product.price === "number" && product.price > 0 ? "#D97706" : isUpcoming ? "#64748B" : "#94A3B8",
          margin: "0 0 12px",
        }}>
          {formattedPrice}
        </p>
        ...
        {/* Made in India */}
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(148,163,184,0.1)", fontSize: 10, color: "#94A3B8", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
          🇮🇳 Made in India
        </div>
        </div>
      </div>
