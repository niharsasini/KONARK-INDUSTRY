...
export default function ProductImageGallery({ images, productName, isNew }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div
        style={{ background: "#0c1525", border: "1px solid #1c3050", borderRadius: 20, padding: 32, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 340, position: "relative", cursor: "zoom-in" }}
        onClick={() => setIsZoomed(true)}
      >
        {isNew && (
          <span style={{ position: "absolute", top: 16, left: 16, background: "#38bdf8", color: "#080f1e", fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 4, textTransform: "uppercase" }}>NEW</span>
        )}
        ...
        {images.length > 1 && (
          <>
            <button onClick={(e) => { e.stopPropagation(); setActiveImage((i) => (i - 1 + images.length) % images.length); }} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(0,0,0,0.5)", border: "1px solid #1c3050", borderRadius: "50%", width: 32, height: 32, color: "#f1f5f9", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
            <button onClick={(e) => { e.stopPropagation(); setActiveImage((i) => (i + 1) % images.length); }} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(0,0,0,0.5)", border: "1px solid #1c3050", borderRadius: "50%", width: 32, height: 32, color: "#f1f5f9", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
          </>
        )}
        <div style={{ position: "relative", width: "85%", height: 280, filter: "drop-shadow(0 4px 20px rgba(56,189,248,0.15))" }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
          {images.map((img, idx) => (
            <button
              ...
              style={{ width: 64, height: 64, borderRadius: 8, border: `2px solid ${activeImage === idx ? "#38bdf8" : "#1c3050"}`, background: "#0e1928", cursor: "pointer", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", padding: 4, transition: "border-color 0.2s", position: "relative" }}
            >
      ...
      {isZoomed && (
        ...
        >
          <button onClick={() => setIsZoomed(false)} style={{ position: "absolute", top: 20, right: 24, background: "transparent", border: "1px solid #1c3050", color: "#f1f5f9", fontSize: 20, width: 40, height: 40, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); setActiveImage((i) => (i - 1 + images.length) % images.length); }} style={{ position: "absolute", left: 20, top: "50%", transform: "translateY(-50%)", background: "rgba(0,0,0,0.6)", border: "1px solid #1c3050", borderRadius: "50%", width: 44, height: 44, color: "#f1f5f9", cursor: "pointer", fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
