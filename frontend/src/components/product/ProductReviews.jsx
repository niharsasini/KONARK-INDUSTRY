...
export default function ProductReviews({ slug }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 680 }}>
      {/* Submit form */}
      {isLoggedIn && (
        <div style={{ background: "var(--bg-card)", border: "1px solid rgba(15,76,129,0.2)", borderRadius: 12, padding: 20, marginBottom: 8 }}>
          <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text-heading)", margin: "0 0 14px" }}>Write a Review</p>
          <form onSubmit={handleSubmitReview} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} required rows={3} placeholder="Share your experience with this product..." style={{ width: "100%", background: "#fff", border: "1px solid var(--border-light)", borderRadius: 8, padding: "10px 14px", color: "var(--text-heading)", fontSize: 13, outline: "none", boxSizing: "border-box", resize: "vertical", fontFamily: "inherit" }} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--navy)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--border-light)")} />
            <button type="submit" disabled={submitting} style={{ padding: "10px 24px", background: "var(--navy)", color: "#fff", fontWeight: 700, fontSize: 13, borderRadius: 8, border: "none", cursor: submitting ? "not-allowed" : "pointer", width: "fit-content", opacity: submitting ? 0.7 : 1 }}>
      ...
      {/* Reviews list */}
      {loading ? (
        <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>Loading reviews...</div>
      ) : reviews.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 24px" }}>
          <p style={{ fontSize: 20, marginBottom: 8 }}>⭐</p>
          <p style={{ fontSize: 14, color: "var(--text-muted)" }}>No reviews yet. Be the first to review this product!</p>
          {!isLoggedIn && <p style={{ fontSize: 13, color: "var(--text-subtle)", marginTop: 8 }}><Link href="/login" style={{ color: "var(--navy)" }}>Sign in</Link> to write a review.</p>}
        </div>
      ) : (
        reviews.map((r, i) => (
          <div key={r.id || i} className="review-item" style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "20px" }}>
            ...
            <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0, lineHeight: 1.7 }}>{r.comment || r.text}</p>
          </div>
        ))
      )}
    </div>
