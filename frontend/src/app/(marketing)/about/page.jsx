  ...
  return (
    <div style={{ background: "linear-gradient(135deg, #050a14 0%, #080f1e 40%, #050a14 100%)", minHeight: "100vh", paddingTop: "calc(64px + var(--banner-h, 0px))" }}>
      ...
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }} className="about-grid">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {STATS.map((s) => (
              <div key={s.label} style={{ background: "#0c1525", border: "1px solid #1c3050", borderTop: "2px solid #38bdf8", borderRadius: 14, padding: "24px 20px", textAlign: "center" }}>
      ...
      <section style={{ background: "#0e1928", padding: "80px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0, position: "relative", maxWidth: 680, margin: "0 auto" }}>
            <div style={{ position: "absolute", left: 47, top: 0, bottom: 0, width: 2, background: "#1c3050" }} />
            {TIMELINE.map((item, i) => (
              <div key={item.year} style={{ display: "flex", gap: 20, alignItems: "flex-start", paddingBottom: 28, position: "relative" }}>
                <div style={{ flexShrink: 0, width: 96, display: "flex", justifyContent: "flex-end", paddingTop: 2 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#38bdf8" }}>{item.year}</span>
                </div>
                <div style={{ width: 12, height: 12, borderRadius: "50%", background: "#38bdf8", border: "2px solid #080f1e", flexShrink: 0, marginTop: 4, zIndex: 1 }} />
      ...
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="mv-grid">
          {[
            ...
            <div key={item.title} style={{ background: "rgba(15,23,42,0.8)", backdropFilter: "blur(12px)", border: "1px solid #1c3050", borderRadius: 20, padding: "32px 28px" }}>
      ...
      <section style={{ background: "#0e1928", padding: "80px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {VALUES.map((v) => (
              <div key={v.title} style={{ background: "#0c1525", border: "1px solid #1c3050", borderRadius: 14, padding: "24px 20px" }}>
                <h3 style={{ fontSize: 17, fontWeight: 700, color: "#38bdf8", marginBottom: 8 }}>{v.title}</h3>
      ...
      <section style={{ padding: "60px 24px", textAlign: "center" }}>
        ...
        <Link
          href="/contact"
          style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 28px", border: "1px solid #1c3050", borderRadius: 8, color: "#f1f5f9", fontSize: 14, fontWeight: 600, textDecoration: "none", transition: "all 0.2s" }}
